...
static inline void EndOverCostAction(int64_t useTimeTick,
...
static inline int64_t CalcId(SnowFlakeWorker *worker) {
  uint64_t result = (worker->_LastTimeTick << worker->_TimestampShift) |
...
extern int64_t WorkerM1NextId(SnowFlakeWorker *worker) {
  ...
  int64_t id =
      worker->_IsOverCost ? NextOverCostId(worker) : NextNormalId(worker);
