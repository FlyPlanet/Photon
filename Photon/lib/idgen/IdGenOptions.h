...
typedef struct IdGenOptions {
  ...
  /// 机器码位长，默认值6，取值范围 [1,
  /// 15]（要求：序列数位长+机器码位长不超过22）
  uint8_t WorkerIdBitLength;

  /// 序列数位长，默认值6，取值范围 [3,
  /// 21]（要求：序列数位长+机器码位长不超过22）
  uint8_t SeqBitLength;

  /// 最大序列数（含），设置范围 [MinSeqNumber,
  /// 2^SeqBitLength-1]，默认值0，表示最大序列数取最大值（2^SeqBitLength-1]）
  uint32_t MaxSeqNumber;

  /// 最小序列数（含），默认值5，取值范围 [5,
  /// MaxSeqNumber]，每毫秒的前5个序列数对应编号0-4是保留位，其中1-4是时间回拨相应预留位，0是手工新值预留位
