...
extern void SetOptions(IdGeneratorOptions options) {
  ...
  if (options.BaseTime == 0) {
    _idGenerator->Worker->BaseTime = 1582136402000;
             ...
             options.BaseTime > GetCurrentTime()) {
  ...
  if (options.SeqBitLength + options.WorkerIdBitLength > 22) {
    ...
    _idGenerator->Worker->WorkerIdBitLength =
        options.WorkerIdBitLength <= 0 ? 6 : options.WorkerIdBitLength;
  }
  ...
  if (options.SeqBitLength < 2 || options.SeqBitLength > 21) {
    ...
    _idGenerator->Worker->SeqBitLength =
        options.SeqBitLength <= 0 ? 6 : options.SeqBitLength;
  }
  ...
  if (options.MaxSeqNumber > maxSeqNumber) {
    ...
    _idGenerator->Worker->MaxSeqNumber =
        options.MaxSeqNumber <= 0 ? maxSeqNumber : options.MaxSeqNumber;
  }
  ...
  if (options.MinSeqNumber < 5 || options.MinSeqNumber > maxSeqNumber) {
    ...
    _idGenerator->Worker->MinSeqNumber =
        options.MinSeqNumber <= 0 ? 5 : options.MinSeqNumber;
  }

  // 7.Others
  _idGenerator->Worker->TopOverCostCount =
      options.TopOverCostCount <= 0 ? 2000 : options.TopOverCostCount;
